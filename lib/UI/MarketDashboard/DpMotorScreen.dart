...
class DpMotorScreen extends StatefulWidget {
  ...
  const DpMotorScreen({Key? key, required this.id, required this.gameType, required this.isOnlyClose, required this.marketName}) : super(key: key);
...
class _DpMotorScreenState extends State<DpMotorScreen> {
  ...
  GlobalKey<FormState> dpmKey = GlobalKey<FormState>();
    ...
    return Scaffold(
                ...
                BlocListener<PanaBloc, PanaState>(
                  ...
                  child: Padding(
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () {
                        if (dpmKey.currentState!.validate()) {
                          numbers.clear();
                          context.read<BidBloc>().add(InitialBidEvent());
                          context.read<PanaBloc>().add(PanaListEvent("dpm", ""));
      ...
      bottomNavigationBar: BlocConsumer<BidBloc, BidState>(
        listener: (context, bidState) {
          debugPrint("[DP Motor Screen] Bid State:- $bidState");
