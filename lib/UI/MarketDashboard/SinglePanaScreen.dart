...
class SinglePanaScreen extends StatefulWidget {
  ...
  const SinglePanaScreen({Key? key, required this.id, required this.gameType, required this.isOnlyClose, required this.marketName}) : super(key: key);
...
class _SinglePanaScreenState extends State<SinglePanaScreen> {
  ...
  void initState() {
    ...
    context.read<PanaBloc>().add(PanaListEvent("sp", "1"));
                                ...
                                child: GridView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    return GestureDetector(
                                      onTap: () {
                                        if (index == 9) {
                                          context.read<PanaBloc>().add(PanaListEvent("sp", "0"));
                                          choosemultiSelect("0");
                                        } else {
                                          context.read<PanaBloc>().add(PanaListEvent("sp", "${index + 1}"));
                                        ...
                                        child: TextFormField(
                                          ...
                                          decoration: InputDecoration(
                                            ...
                                            prefixIcon: Container(
                                              ...
                                              decoration: BoxDecoration(borderRadius: BorderRadius.circular(8), color: AppTheme().greyColor),
                                              child: Center(
                                                child: Text(
                            ...
                            bidTextField(
                              ...
                              onChanged: (digitVal) {
                                ...
                                if (cleaned.length > 3) {
                                  ...
                                  Fluttertoast.showToast(msg: "Digits must be less than 3 characters");
                                  return;
                                }

                                // Check for unique digits
                                final uniqueDigits = cleaned.split('').toSet();
                                if (uniqueDigits.length != cleaned.length) {
                                  digitController.clear();
                                  Fluttertoast.showToast(msg: "Digits must be unique.");
                                  return;
                                }
                                ...
                                for (int i = 0; i < digits.length - 1; i++) {
                                  if (digits[i] > digits[i + 1]) {
                                    digitController.clear();
                                    Fluttertoast.showToast(msg: "Each digit must be smaller than the next.");
                                    return;
                                  }
                                }
                                  ...
                                  for (int i = 0; i < _controller.length; i++) {
                                    ...
                                    if (int.parse(settingState.model.data![0].minimumSpDpTpBidAmount.toString()) <= int.parse(item.amount)) {
                                      BlocProvider.of<BidBloc>(
                                        context,
                                      ).add(AddBidEvent(mType: "sp", digit: item.digit, price: item.amount, status: gameType, pana: ""));
                                    } else {
        ...
        children: [
          ...
          BlocConsumer<BidBloc, BidState>(
            listener: (context, bidState) {
              debugPrint("[Single Pana Screen] Bid State:- $bidState");
