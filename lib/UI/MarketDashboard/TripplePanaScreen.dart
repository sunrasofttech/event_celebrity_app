...
class TriplePanaScreen extends StatefulWidget {
  ...
  const TriplePanaScreen({Key? key, required this.id, required this.gameType, required this.isOnlyClose, required this.marketName}) : super(key: key);
...
class _TriplePanaScreenState extends State<TriplePanaScreen> {
  ...
  void initState() {
    ...
    context.read<PanaBloc>().add(PanaListEvent("tp", ""));
    //multiSelect("1");
    setState(() {
                          ...
                          children: [
                            bidTextField(
                              ...
                              onChanged: (digitVal) {
                                ...
                                // Only validate if 3 digits are entered
                                if (cleaned.length == 3) {
                                  final allSame = cleaned.split('').every((char) => char == cleaned[0]);

                                  if (!allSame) {
                                    digitController.clear();
                                    Fluttertoast.showToast(msg: "All digits must be the same (e.g. 111, 222)");
                                    return;
                                  }
                                }
                                  ...
                                  for (int i = 0; i < _controller.length; i++) {
                                    ...
                                    if (int.parse(settingState.model.data![0].minimumSpDpTpBidAmount.toString()) <= int.parse(item.amount)) {
                                      BlocProvider.of<BidBloc>(
                                        context,
                                      ).add(AddBidEvent(mType: "tp", digit: item.digit, price: item.amount, status: gameType, pana: ""));
                                    } else {
        ...
        children: [
          ...
          BlocConsumer<BidBloc, BidState>(
            listener: (context, bidState) {
              debugPrint("[Triple Pana Screen] Bid State:- $bidState");
                      ...
                      child: GestureDetector(
                        onTap: () {
                          ...
                          if (state is UserProfileFetchedState) {
                            ...
                            if (double.parse(state.user.data?.balance.toString() ?? "0").truncate() >= double.parse(bidState.total).truncate()) {
                              if (selectItem.isNotEmpty) {
                                setState(() {
                                  loading = true;
